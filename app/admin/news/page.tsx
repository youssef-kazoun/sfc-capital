  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-2xl text-ink">Manage news</h1>
        <Link href="/admin/news/new">
          <Button><span className="flex items-center gap-1.5"><Plus className="h-4 w-4" /> New article</span></Button>
        </Link>
      </div>
      <div className="rounded-sm border border-line bg-white divide-y divide-line">
        {rows.map((r) => (
          <div key={r.id} className="flex items-center justify-between px-4 py-3">
            ...
            <Badge tone={r.published ? "gain" : "neutral"}>{r.published ? "Published" : "Draft"}</Badge>
            <button onClick={() => remove(r.id)} className="ml-4 text-slate hover:text-loss">
              <Trash2 className="h-4 w-4" />
            </button>
          </div>
        ))}
        {rows.length === 0 && <p className="p-6 text-slate text-sm">No articles yet.</p>}
      </div>
    </div>
  );
}
