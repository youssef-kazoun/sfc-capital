...
export default function ContactPage() {
  ...
  if (status === "sent") {
    return (
      <div className="mx-auto max-w-lg px-4 py-24 text-center">
        <h1 className="font-display text-2xl text-ink">تم إرسال الرسالة</h1>
        <p className="text-slate mt-2">سنرد عليك في أقرب وقت.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-16">
      <h1 className="font-display text-2xl text-ink mb-6">تواصل معنا</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="text-xs text-slate">الهاتف (اختياري)</label>
        ...
        {status === "error" && <p className="text-sm text-loss">حدث خطأ ما. حاول مرة أخرى.</p>}
        <Button type="submit" className="w-full" disabled={status === "busy"}>
          {status === "busy" ? "جاري الإرسال..." : "إرسال الرسالة"}
        </Button>
      </form>
    </div>
  );
