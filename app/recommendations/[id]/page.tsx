  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12">
      ...
      <div className="grid grid-cols-3 gap-4 mt-6 mb-8">
        <Stat label="الدخول" value={rec.entryPrice} />
        <Stat label="الهدف" value={rec.targetPrice} tone="text-gain" />
        <Stat label="وقف الخسارة" value={rec.stopLoss} tone="text-loss" />
      </div>

      <div className="flex gap-4 text-sm text-slate mb-6">
        <span>المخاطرة: {RISK_LABELS[rec.riskLevel] || rec.riskLevel}</span>
        <span>المدى الزمني: {rec.timeHorizon}</span>
        {author && <span>بقلم {author.name}</span>}
      ...
      {updates.length > 0 && (
        <div className="mt-10">
          <h2 className="font-display text-lg text-ink mb-4">التحديثات</h2>
